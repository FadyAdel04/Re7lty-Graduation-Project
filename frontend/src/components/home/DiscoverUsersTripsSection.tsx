...
const DiscoverUsersTripsSection = () => {
  ...
  return (
    <section className="py-16 md:py-24 bg-background relative overflow-hidden">
      {/* Decorative blobs */}
      <div className="absolute top-0 right-0 -mr-20 -mt-20 w-60 h-60 md:w-80 md:h-80 rounded-full bg-primary/10 blur-3xl opacity-60"></div>
      <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-60 h-60 md:w-80 md:h-80 rounded-full bg-secondary/10 blur-3xl opacity-60"></div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          <div className="flex-1 text-center lg:text-right space-y-6 md:space-y-8">
            ...
            <p className="text-muted-foreground text-base md:text-xl leading-relaxed max-w-lg mx-auto lg:mx-0">
            ...
            <div className="flex flex-wrap gap-4 justify-center lg:justify-start">
               <Link to="/timeline">
                 <Button className="h-12 md:h-14 px-6 md:px-8 rounded-full bg-primary text-primary-foreground hover:bg-primary-hover shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 text-base md:text-lg">
                   تصفح يوميات السفر
            ...
            <div className="pt-8 border-t border-border flex items-center justify-center lg:justify-start gap-8">
               <div className="text-center">
                 <div className="text-2xl md:text-3xl font-black text-foreground">+5000</div>
                 <div className="text-sm text-muted-foreground">رحلة موثقة</div>
               </div>
               <div className="w-px h-12 bg-border"></div>
               <div className="text-center">
                 <div className="text-2xl md:text-3xl font-black text-foreground">+10K</div>
                 <div className="text-sm text-muted-foreground">صورة ومقطع</div>
               </div>
            </div>
          </div>
          ...
          <div className="flex-1 w-full relative h-[400px] md:h-[500px] flex items-center justify-center mt-8 lg:mt-0">
            ...
            <div className="relative w-full h-full max-w-[500px] transform scale-75 sm:scale-90 md:scale-100 transition-transform duration-300">
               {collageItems.map((item, index) => (
                 <div 
                    key={item.id}
                    className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 md:w-72 bg-card p-3 rounded-2xl shadow-xl transition-all duration-500 hover:z-50 hover:scale-105 cursor-pointer border border-border ${item.rotation}`}
                    style={{ 
                 ...
                 >
                    ...
                    <div className="flex items-center justify-between">
                       <div>
                          ...
                          <span className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5"><User className="h-3 w-3" /> {item.user}</span>
                       </div>
               ...
               <div className="absolute inset-0 border-2 border-dashed border-border rounded-full animate-spin-slow opacity-30 pointer-events-none" style={{ animationDuration: '30s' }}></div>
            </div>
          </div>
