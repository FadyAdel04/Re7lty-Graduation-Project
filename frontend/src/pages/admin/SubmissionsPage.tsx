    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto space-y-10" dir="rtl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
           <div>
              <h1 className="text-3xl font-black text-foreground mb-2 font-cairo">طلبات <span className="text-indigo-600">التدقيق</span></h1>
              <p className="text-muted-foreground font-bold text-sm">مراجعة والتحقق من أوراق الشركات الراغبة في الانضمام للمنصة.</p>
           </div>
        </div>
             ...
             <button
               ...
               className={cn(
                 ...
                 filter === btn.id 
                   ...
                   : "bg-card border border-border text-muted-foreground hover:bg-indigo-50 hover:text-indigo-600"
               )}
             >
               {btn.label}
               <span className={cn(
                 "px-2 py-0.5 rounded-lg text-[10px] font-black",
                 filter === btn.id ? "bg-card/20 text-white" : "bg-muted text-gray-400"
               )}>
                 {btn.count}
               </span>
             </button>
        ...
        {loading ? (
          <div className="grid gap-6">
             {[1,2,3].map(i => <div key={i} className="h-64 bg-card rounded-[2.5rem] animate-pulse shadow-sm" />)}
          </div>
          ...
          <div className="text-center py-32 bg-card rounded-[2.5rem] border border-gray-50 shadow-sm">
             ...
             <h3 className="text-xl font-black text-foreground">لا توجد طلبات للعرض</h3>
                     ...
                     <Card className="border-0 shadow-2xl shadow-gray-200/40 rounded-[2.5rem] overflow-hidden group hover:scale-[1.01] transition-all duration-500">
                        <CardContent className="p-0">
                           <div className="flex flex-col lg:flex-row">
                             ...
                             {/* Side Status Bar */}
                             <div className={cn("w-full lg:w-3 border-b lg:border-b-0 lg:border-l border-border", status.bg.replace('/50', ''))} />
                             
                             <div className="flex-1 p-8">
                                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                                   <div className="flex items-center gap-5">
                                      <div className="w-16 h-16 rounded-[1.25rem] bg-muted flex items-center justify-center text-indigo-600 font-black text-2xl shadow-inner group-hover:scale-110 transition-transform duration-500">
                                         {s.companyName[0]}
                                      </div>
                                      <div>
                                         <h3 className="text-2xl font-black text-foreground mb-1">{s.companyName}</h3>
                                   ...
                                   <div className="flex items-center gap-2">
                                      {s.status === 'pending' ? (
                                        ...
                                        <Button 
                                          ...
                                          className="h-12 px-6 rounded-2xl bg-muted text-gray-400 hover:bg-rose-50 hover:text-rose-600 border-0 font-black text-xs"
                                        >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                                   {[
                                     ...
                                     { icon: Building2, label: 'التخصص', value: s.tripTypes },
                                      ...
                                      <div key={i} className="p-4 rounded-3xl bg-muted/50 border border-border/50 hover:bg-card transition-colors group/item">
                                         <div className="flex items-center gap-3 mb-2">
                                            <div className="w-8 h-8 rounded-xl bg-card flex items-center justify-center shadow-sm group-hover/item:scale-110 transition-transform">
                                ...
                                {s.message && (
                                   <div className="p-6 rounded-3xl bg-indigo-50/50 border border-indigo-100/30">
                                      ...
                                      <p className="text-sm font-bold text-muted-foreground leading-relaxed">{s.message}</p>
                                   </div>
                                )}
