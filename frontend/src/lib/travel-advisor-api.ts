...
export async function getHotels(city: string, budget?: string, checkIn?: string, checkOut?: string, lat?: number, lon?: number, locationId?: string): Promise<TravelAdvisorHotel[]> {
  try {
    // Build URL with lat/lng and dates as primary params for TripAdvisor16 API
    ...
    const data = await response.json();
    console.log(`[getHotels] TripAdvisor data for ${city}:`, data);

    // Backend returns a normalized array directly
    let hotelList: any[] = Array.isArray(data) ? data : (data?.data || data?.result || []);

    let formattedHotels: TravelAdvisorHotel[] = hotelList
      .filter(h => h && h.name)
      .map((h: any, idx: number) => ({
        location_id: h.location_id?.toString() || `hotel_${idx}`,
        name: h.name,
        rating: h.rating?.toString() || '4.5',
        num_reviews: h.num_reviews?.toString() || '0',
        latitude: h.latitude?.toString(),
        longitude: h.longitude?.toString(),
        photo: h.photo || {
          images: {
            medium: { url: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=500&q=80' },
            large: { url: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1000&q=80' }
          }
        },
        address: h.address || city,
        price: h.price?.toString() || 'اضغط لرؤية السعر',
        amenities: h.amenities || [{ name: 'Wi-Fi' }],
        website: h.website || '',
        review_word: h.review_word || '',
        star_rating: h.star_rating || 0,
        is_free_cancellable: h.is_free_cancellable || false,
        distance_to_center: h.distance_to_center || '',
      }));

    // Sort by budget preference
    if (budget === 'low') {
      formattedHotels.sort((a, b) => parseFloat(a.price?.replace(/[^\d.]/g, '') || "9999") - parseFloat(b.price?.replace(/[^\d.]/g, '') || "9999"));
    } else if (budget === 'high') {
      formattedHotels.sort((a, b) => parseFloat(b.price?.replace(/[^\d.]/g, '') || "0") - parseFloat(a.price?.replace(/[^\d.]/g, '') || "0"));
    }
...
export async function getTripPlan(city: string, days: number = 3, budgetLevel?: "low" | "medium" | "high", checkIn?: string, checkOut?: string, lat?: number | null, lon?: number | null, locationId?: string): Promise<TripPlan | null> {
  try {
    ...
    // Hotels use Booking.com with coordinates (lat/lng)
    const finalLat = lat || (GOVERNORATES_COORDINATES[city]?.lat);
    const finalLon = lon || (GOVERNORATES_COORDINATES[city]?.lng);

    console.log(`[getTripPlan] Using Booking.com with coords: ${finalLat},${finalLon} for hotels`);
    const hotels = await getHotels(city, budgetLevel, checkIn, checkOut, finalLat || undefined, finalLon || undefined);
