# 🌍 Re7lty – شارك رحلتك | Share Your Journey

## 🧭 نظرة عامة (Overview)

**Re7lty** هو موقع تفاعلي يربط بين عشاق السفر حول العالم، حيث يمكن للمستخدمين مشاركة رحلاتهم بالصور والفيديوهات وتفاصيل المسار على خريطة تفاعلية.  
كما يمكنهم استكشاف رحلات الآخرين، تعديلها، ومقارنة الخطط، مما يجعل **Re7lty** مزيجًا بين **Social Media** و **Travel Recommendation System**.

---

## ✨ المميزات (Features)

### 🧳 1. إنشاء رحلة جديدة (Create Trip)
- إضافة الوجهة (Destination)
- تحديد مدة الرحلة (Days)
- كتابة تفاصيل كل يوم (Day 1, Day 2...)
- تحديد الأماكن (مطاعم، متاحف، شواطئ...)
- رفع صور وفيديوهات الرحلة  
📍 يتم عرض كل موقع على الخريطة باستخدام **Marker تفاعلي**.

---

### 🗺️ 2. عرض الرحلة على خريطة تفاعلية
- خريطة تعرض مسار الرحلة من البداية للنهاية.
- الضغط على أي نقطة يظهر تفاصيل المكان.
- أنيميشن جميل لمسار الرحلة (Line Tracing Animation).

---

### 🧠 3. AI Suggestion Assistant
- الذكاء الاصطناعي يقترح رحلات بناءً على:
  - الموقع الحالي 🗺️  
  - عدد الأيام 🕒  
  - الميزانية 💰  
مثال: *“اقتراح رحلة 3 أيام في الإسكندرية تشمل المعالم + مطاعم بحرية.”*

---

### 💬 4. التفاعل الاجتماعي (Social Interaction)
- إعجابات ❤️ وتعليقات 💬 على الرحلات.
- متابعة المسافرين الآخرين 👥.
- صفحة “Top Travelers” لعرض أكثر المسافرين نشاطًا وتميّزًا.

---

### 🧭 5. صفحة الاستكشاف (Explore Page)
- بحث عن الرحلات حسب:
  - الدولة أو المدينة
  - نوع الرحلة (مغامرة / استجمام / عائلية / رومانسية)
  - الميزانية أو المدة
- خريطة عالمية تفاعلية تحتوي على **نقاط رحلات المستخدمين (Clickable Pins)**.
- فورم ذكي لتحديد نقطة البداية والنهاية والمدة والميزانية لتوليد ترشيحات مناسبة لك.

---

### 🎞️ 6. قسم الذكريات (Travel Memories)
- إنشاء فيديو تلقائي من صور الرحلة (Slideshow Generator).
- إضافة موسيقى، نصوص، وعناوين.
- تصدير الفيديو كـ **"Travel Memory Clip"**.

---

### 👤 7. صفحة المستخدم (User Profile)
- عدد الرحلات المنشورة ✈️  
- الدول التي زارها (مع خريطة صغيرة تظللها 🗺️)  
- عدد المتابعين 👥  
- مستوى المستخدم “Explorer Level” حسب نشاطه  

---

### 🗓️ 8. إضافات مميزة
- **تقويم الرحلات** لكل مستخدم 📆  
- **حساب الميزانية المتوقعة** 🧾  
- **عرض الطقس المتوقع** أثناء الرحلة 🌦️  
- **اقتراح وسائل المواصلات المحلية** 🚕  
- **Story Mode** لعرض الرحلة في شكل Instagram Story 🎬  

---

## ⚙️ إعداد متغيرات البيئة

1. **الواجهة الأمامية (Vite)**  
   - انسخ الملف [`env.example`](env.example) إلى `.env` في جذر المشروع:  
     ```bash
     cp env.example .env
     ```
   - عدل قيم `VITE_*` حسب مفاتيحك الفعلية (Clerk، عنوان الـ API، Pusher).

2. **الخلفية (Node/Express)**  
   - انسخ نفس الملف إلى داخل مجلد `backend` باسم `.env`:  
     ```bash
     cp env.example backend/.env
     ```
   - حدث المفاتيح غير المسبوقة بـ `VITE_` (مثل `CLERK_SECRET_KEY`، `MONGODB_URI`، مفاتيح Pusher الخاصة بالسيرفر).

> عند النشر على Vercel، أدخل القيم نفسها في صفحة متغيرات البيئة لكل مشروع كما هو موضح في قسم “نشر الـ Backend على Vercel”.

---

## 🧩 التقنيات المستخدمة (Tech Stack)

| الجزء | التقنية |
|-------|-----------|
| **الواجهة الأمامية (Frontend)** | React.js / Next.js + Tailwind CSS + Framer Motion |
| **الخلفية (Backend)** | Node.js + Express |
| **قاعدة البيانات (Database)** | MongoDB أو Firebase Firestore |
| **الخرائط (Maps)** | Google Maps API أو Mapbox |
| **رفع الوسائط (Uploads)** | Cloudinary أو Firebase Storage |
| **تسجيل الدخول (Auth)** | Clerk أو Firebase Auth |
| **الذكاء الاصطناعي (AI)** | OpenAI API أو Gemini API |
| **الاستضافة (Hosting)** | Vercel (Frontend & Backend Serverless Functions) |

---

## 🌍 أماكن الرحلات في النسخة الأولى
- مرسى مطروح  
- الإسكندرية  
- الساحل الشمالي  
- الأقصر  
- أسوان  
- الغردقة  
- شرم الشيخ  

---

## 🧠 فكرة المشروع (Why It’s Creative)
Re7lty مش مجرد موقع لحجز أو مدونة سفر،  
بل هو تجربة اجتماعية تفاعلية تجمع بين:
- التكنولوجيا الحديثة  
- الذكاء الاصطناعي  
- الخرائط  
- التفاعل البصري  

---

## 🚀 نشر الـ Backend على Vercel (Serverless)

> تم تجهيز المشروع ليتوافق مع متطلبات Vercel Functions. اتبع الخطوات التالية:

1. **إعداد التطبيق**
   - تم استخراج تطبيق Express في ملف `[backend/src/app.ts](backend/src/app.ts)`، بينما يستخدم `[backend/src/server.ts](backend/src/server.ts)` هذا التطبيق للتشغيل المحلي فقط.
   - يوجد ملف Serverless جاهز في `[backend/api/index.js](backend/api/index.js)` يصدّر التطبيق من مجلد `dist`.

2. **ضبط المشروع في Vercel**
   - أنشئ مشروعًا جديدًا وأشر إلى المجلد الفرعي `backend` باعتباره **Root Directory**.
   - استخدم الأمر الافتراضي `npm run build` كي يتم توليد ملفات `dist/`، حيث تعتمد الدوال عليها.
   - ملف التكوين `[backend/vercel.json](backend/vercel.json)` يوجه كل الطلبات إلى `api/index.js`.

3. **متغيرات البيئة على Vercel**
   - استخدم الملف [`env.example`](env.example) كمرجع؛ يتضمن قسمًا للواجهة الأمامية وآخر للخلفية.
   - عند إنشاء مشروعين منفصلين على Vercel (Frontend و Backend)، أضف المتغيرات التالية في كل مشروع:
     - **Frontend (Vite Project):**
       ```
       VITE_CLERK_PUBLISHABLE_KEY=...
       VITE_API_URL=https://your-backend.vercel.app
       VITE_PUSHER_KEY=...
       VITE_PUSHER_CLUSTER=...
       ```
     - **Backend (Serverless Project):**
       ```
       CLERK_SECRET_KEY=...
       MONGODB_URI=...
       MONGODB_DB=re7lty
       PUSHER_APP_ID=...
       PUSHER_KEY=...
       PUSHER_SECRET=...
       PUSHER_CLUSTER=...
       ```

4. **الإشعارات اللحظية (Pusher)**
   - تم استبدال نظام SSE بـ [Pusher](https://pusher.com/). قم بإنشاء تطبيق Pusher جديد واحصل على المفاتيح، ثم حدث ملف `.env` (و `Vercel env`) بالإضافة إلى مفاتيح الواجهة الأمامية:
     ```
     VITE_PUSHER_KEY=...
     VITE_PUSHER_CLUSTER=...
     ```
   - يعمل الـ Backend على بث الإشعارات عبر القناة `user-{clerkId}`، بينما يستمع الـ Frontend إليها في `NotificationBell`.

5. **تشغيل محلي**
   - ما زال بإمكانك تشغيل الخادم محليًا عبر:
     ```bash
     cd backend
     npm install
     npm run dev
     ```
   - ولتجربة نسخة Vercel محليًا يمكنك تشغيل:
     ```bash
     npm run build
     vercel dev
     ```

---
